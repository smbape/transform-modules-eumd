...
import _varname from "./_varname";

const strictModeProgram = strictMode().visitor.Program;
...
export default function() {
  const REASSIGN_REMAP_SKIP = Symbol("REASSIGN_REMAP_SKIP");
    ...
    visitor: {
      ...
      Program: {
        exit(path, state) {
          ...
          const noInterop = Boolean(opts.noInterop);
          ...
          const addRequire = (sourceNode, blockHoist) => {
            ...
            const ref = path.scope.generateUidIdentifier(_varname(this, t, sourceNode, _resolveName));

            const requireNode = buildRequire(t.stringLiteral(source));
            requireNode.expression._fromImport = true;
            const varDecl = t.variableDeclaration("var", [
              t.variableDeclarator(ref, requireNode.expression)
            ]);
            ...
            return requires[source] = ref; // eslint-disable-line no-return-assign
          };
          ...
          for (const path of body) {
            ...
            if (path.isImportDeclaration()) {
              ...
              if (source) {
                const ref = addRequire(source, path.node._blockHoist);
              ...
              const exportNode = buildExportAll({
                OBJECT: addRequire(path.node.source, path.node._blockHoist)
              });
          ...
          for (const source in imports) { // eslint-disable-line guard-for-in
            const {specifiers, maxBlockHoist, node: sourceNode} = imports[source];
            if (specifiers.length) {
              const uid = addRequire(sourceNode, maxBlockHoist);
                ...
                if (t.isImportSpecifier(specifier)) {
                  ...
                  if (specifier.imported.name === "default") {
                    if (wildcard) {
                      ...
                      target = wildcard = path.scope.generateUidIdentifier(uid.name); // eslint-disable-line no-multi-assign
                      const varDecl = t.variableDeclaration("var", [
                        t.variableDeclarator(
                          target,
                          t.callExpression(
          ...
          if (!hasImports && !hasExports && opts.esmOnly) {
            this.notEsm = true;
            return;
          }

          strictModeProgram(path, state);
